...
public class EnemyManager : MonoBehaviour
{
    ...
    IEnumerator AI_Loop(EnemyBehaviour enemy){
        ...
        EnemyBehaviour attackingEnemy = RandomEnemyExcludingOne(enemy);
    ...
    public EnemyBehaviour RandomEnemy() {
    ...
    public EnemyBehaviour RandomEnemyExcludingOne(EnemyBehaviour ex) {
        enemyIndexes = new List<int>();

        for(int i = 0; i < allEnemies.Length; i++) {
            if(allEnemies[i].enemyAvailability && allEnemies[i].enemyScript != ex)
                enemyIndexes.Add(i);
        }
